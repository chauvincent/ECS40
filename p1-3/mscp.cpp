...
////		ECS40 - Winter 2014 - HOMEWORK #3						///
////		Coverting mscp.c to mscp.cpp PART 2						///
////																///
////																///
////																///	
/*----------------------------------------------------------------------+
...
string mscp_c_rcsid[] = "@(#)$Id: mscp.c,v 1.18 2003/12/14 15:12:12 marcelk Exp $";
...
//changed these from static to extern to test
byte castle[64]; /* Which pieces may participate in castling */

int computer[2]; /* Which side is played by the computer */

int xboard_mode; /* XBoard protocol, surpresses prompt */	

unsigned long nodes; /* Node counter */
...
static struct side white, black, *friendly, *enemy;
...
const int PRESCORE_EQUAL      = (10U<<9);

const int PRESCORE_HASHMOVE   = (3U<<14);

const int PRESCORE_KILLERMOVE = (2U<<14);

const int PRESCORE_COUNTERMOVE =(1U<<14);
...
//static union {
struct transpositionTable {                     /* Transposition table entry */
...
struct bookEntry {                     /* Opening book entry */
...
transpositionTable *const TTABLE = core.tt;

bookEntry *const BOOK = core.bk;
...
static void print_move_long(int move)
{
        ...
        if (move >= SPECIAL) {
                ...
                if ((bboard[FR(move)] == WHITE_PAWN && R(TO(move)) == RANK_8)
                ||  (bboard[FR(move)] == BLACK_PAWN && R(TO(move)) == RANK_1)) {
...
static void print_board(void)
{
        ...
        for (rank=RANK_8; rank>=RANK_1; rank--) {
                ...
                for (file=FILE_A; file<=FILE_H; file++) {
                        ...
                        putchar(PIECE2CHAR(bboard[SQ(file,rank)]));
                }
                putchar('\n');
        }
        ...
        cout << ( bboard[CASTLE] & CASTLE_WHITE_KING ? "K" : "");

        cout << ( bboard[CASTLE] & CASTLE_WHITE_QUEEN ? "Q" : "");

        cout << ( bboard[CASTLE] & CASTLE_BLACK_KING ? "k" : "" );

        cout << ( bboard[CASTLE] & CASTLE_BLACK_QUEEN ? "q" : "") << " ";
        
        if (bboard[EP]) print_square(bboard[EP]);
...
static void setup_board(const char *fen)
{
       ...
       // memset(board, 0, sizeof board);
        ...
        while (*fen) {
                ...
                case 'K': bboard[CASTLE] |= CASTLE_WHITE_KING; break;
 
                case 'Q': bboard[CASTLE] |= CASTLE_WHITE_QUEEN; break;
 
                case 'k': bboard[CASTLE] |= CASTLE_BLACK_KING; break;
 
                case 'q': bboard[CASTLE] |= CASTLE_BLACK_QUEEN; break;
 
                case 'a': case 'b': case 'c': case 'd':
 
                case 'e': case 'f': case 'g': case 'h':
 
                        bboard[EP] = SQ(CHAR2FILE(*fen),WTM()?RANK_5:RANK_4);
...
static void make_move(int move)
{
        ...
        *undo_sp++ = -1;                        /* Place sentinel */
        ...
        if (bboard[EP]) {                        /* Clear en-passant info */
        ...
        if (move & SPECIAL) {                   /* Special moves first */
                ...
                case RANK_7:
                        ...
                        if (bboard[fr] == BLACK_PAWN) { /* Set en-passant flag */
                ...
                case RANK_2:
                        if (bboard[fr] == WHITE_PAWN) { /* Set en-passant flag */
        ...
        ply++;
        if (bboard[to]!=EMPTY ||
            bboard[fr]==WHITE_PAWN || bboard[fr]==BLACK_PAWN
        ) {
        ...
        *undo_sp++ = fr;
        ...
        if (bboard[CASTLE] & (castle[fr] | castle[to])) {
        
                *undo_sp++ = bboard[CASTLE];
        
                *undo_sp++ = CASTLE;
        
                bboard[CASTLE] &= ~(castle[fr] | castle[to]);
        }
}
...
static int push_move(int fr, int to)
{
        ...
        /* what do we capture */
        if (bboard[to] != EMPTY) {
                prescore += (1<<9) + prescore_piece_value[bboard[to]];
        }

        /* does the destination square look safe? */
        if (WTM()) {
                if (black.attack[to] != 0) { /* defended */
                        prescore -= prescore_piece_value[bboard[fr]];
                }
        } else {
...
static int test_illegal(int move)
{
        ...
        return friendly->attack[enemy->king] != 0;
}
...
static void generate_moves(unsigned treshold)
{
        ...
        for (fr=0; fr<64; fr++) {
                ...
                case BLACK_KING:
                        ...
                        do {
                                ...
                                to = fr+king_step[dir];
                                if (bboard[to] != EMPTY &&
                                    PIECE_COLOR(bboard[to]) == WTM()) continue;
                                push_move(fr, to);
                        } while (dirs -= dir);
                        break;
                ...
                case BLACK_PAWN:
                        ...
                        if (F(fr) != FILE_H) {
                                ...
                                if (bboard[to] && bboard[to] < BLACK_KING) {
                                        push_pawn_move(fr, to);
                                }
                        }
         ...
         */
        if (bboard[CASTLE] && !enemy->attack[friendly->king]) {
               
                if (WTM() && (bboard[CASTLE] & CASTLE_WHITE_KING) &&
                        !bboard[F1] && !bboard[G1] &&
                        !enemy->attack[F1])
                {
                ...
                if (WTM() && (bboard[CASTLE] & CASTLE_WHITE_QUEEN) &&
                        !bboard[D1] && !bboard[C1] && !bboard[B1] &&
                        !enemy->attack[D1])
                {
                        push_special_move(E1, C1);
                }
               
                if (!WTM() && (bboard[CASTLE] & CASTLE_BLACK_KING) &&
                        !bboard[F8] && !bboard[G8] &&
                        !enemy->attack[F8])
                {
                ...
                if (!WTM() && (bboard[CASTLE] & CASTLE_BLACK_QUEEN) &&
                        !bboard[D8] && !bboard[C8] && !bboard[B8] &&
                        !enemy->attack[D8])
                {
        ...
        if (bboard[EP]) {
                ...
                if (WTM()) {
                        if (F(ep) != FILE_A && bboard[ep-DIR_E] == WHITE_PAWN) {
               
                                if (push_move(ep-DIR_E, ep+DIR_N))
                                        move_sp[-1].move |= SPECIAL;
               
                        }
                        if (F(ep) != FILE_H && bboard[ep+DIR_E] == WHITE_PAWN) {
                        ...
                        if (F(ep) != FILE_A && bboard[ep-DIR_E] == BLACK_PAWN) {
               
                                if (push_move(ep-DIR_E, ep-DIR_N))
                                        move_sp[-1].move |= SPECIAL;
               
                        }
                        if (F(ep) != FILE_H && bboard[ep+DIR_E] == BLACK_PAWN) {
...
static void print_move_san(int move)
{
        ...
        if ((bboard[fr]==WHITE_PAWN) || (bboard[fr] == BLACK_PAWN)) {
                /*
         ...
         */
        putchar(toupper(PIECE2CHAR(bboard[fr])));
        ...
        while (move_sp > moves) {
                ...
                ||  bboard[fr] != bboard[FR(move_sp->move)] /* same piece */
...
check:
        ...
        if (enemy->attack[friendly->king]) {      /* in check, is mate? */
...
static int parse_move(const char *line, int *num)
{
        ...
        const char                    *fr_file = 0;
        
        const char                    *fr_rank = 0;
        
        const char                    *to_file = 0;
        
        const char                    *to_rank = 0;
        
        const char                    *prom_piece = 0;
        ...
        *num = n;
        ...
        while (move_sp > m) {
                ...
                if ((piece && *piece != toupper(PIECE2CHAR(bboard[fr])))
                ...
                if (move) {
                        ...
                        if (piece) {
                                cout << "ambiguous!" << endl; bboard.print_board(); puts(line);
                                move_sp = m;
                                return 0;
                        }
                        /* if this move is a pawn move and the previously
                           found isn't, we accept it */
                        old_is_p = (bboard[FR(move)]==WHITE_PAWN) ||
                                (bboard[FR(move)]==BLACK_PAWN);
                        new_is_p = (bboard[fr]==WHITE_PAWN) ||
                                (bboard[fr]==BLACK_PAWN);
                        ...
                        if (new_is_p && !old_is_p) {
                                ...
                                cout << "ambiguous!"; bboard.print_board(); puts(line);
                                move_sp = m;
                                return 0;
                        }
                } else {
...
{
        const bookEntry *a = (const bookEntry* )ap;

        const bookEntry *b = (const bookEntry* )bp;
...
static void load_book(const char *filename)
{
       ...
       fp = fopen(filename, "r");
        ...
        if (!fp) {
                ...
                cerr << "no opening book: " << filename << endl;
        ...
        while (readline(line, sizeof(line), fp) >= 0) {
                ...
                for (;;) {
                        ...
                        s += num;
                        if (booksize < CORE) {
                                BOOK[booksize].hash = bboard.compute_hash();
...
const int DIFF(int a,int b) { return ( ((a)>(b) ? (a)-(b) : (b)-(a)) ); }
const int TAXI(int a,int b) { return ( DIFF(F(a),F(b)) + DIFF(R(a),R(b)) ); }
...
static int evaluate(void)
{
        ...
        for (sq=0; sq<64; sq++) {
                ...
                if (bboard[sq] == EMPTY) continue;
                score += piece_square[bboard[sq]-1][sq];
...
static int qsearch(int alpha, int beta)
{
        ...
        hash_stack[ply] = bboard.compute_hash();
...
static int search(int depth, int alpha, int beta)
{
        ...
        incheck = enemy->attack[friendly->king];
...
static int root_search(int maxdepth)
{
        ...
        cout << " nodes ply score move " << endl;
        ...
        for (depth = 1; depth <= maxdepth; depth++) {
                ...
                while (m < move_sp) {
                        ...
                        if (friendly->attack[enemy->king] != 0) { /* illegal? */
				...
				cout << " " << setw(5) << nodes << " " << setw(3) << depth << " " << setw(1) << setprecision(2) << (best_score / 100.0) << " ";
...
{
        bboard.setup_board("rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w KQkq -");
...
//static void command mscp_commands[]; /* forward declaration */
        ...
        for (;;) { /* main loop */
                ...
                while (computer[!WTM()]) {
                        ...
                        if (!move || ply >= 1000) {
                                cout << "game over: ";
                                compute_attacks();
                                if (!move && enemy->attack[friendly->king] != 0) {
                                        puts(WTM() ? "0-1" : "1-0");
                                } else {
                                        puts("1/2-1/2");
                                }
